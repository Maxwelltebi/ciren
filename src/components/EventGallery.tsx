...
export default function EventGallery() {
  ...
  return (
    <section className="py-20 lg:py-24 bg-white" id="gallery">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {events.map((event) => (
            ...
            >
              <span className="relative block aspect-[4/3] mx-3 mb-7">
                ...
                <span className="absolute inset-0 rounded-2xl overflow-hidden bg-[#0B0F19] shadow-xl border border-white/20">
                  {event.photos[0] ? (
                    ...
                    <span className="flex h-full items-center justify-center text-center px-8">
                      <span className="font-display text-3xl text-white leading-tight">
                        {event.name}
                      </span>
                    </span>
                  )}
                  <span className="absolute bottom-4 left-4 bg-white px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-[#046e00]">
                    {event.photos.length
              ...
              <span className="block text-xl font-bold text-[#0B0F19] font-display mb-2">
                {event.name}
              </span>
      ...
      {selected && (
        <EventPhotoStack key={selected.id} event={selected} onClose={close} />
      )}
    </section>
  );
}
